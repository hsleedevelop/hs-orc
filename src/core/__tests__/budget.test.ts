...
describe('출처를 섞어 표시하지 않는다', () => {
  it('우선순위는 actual > metered > estimate 다', () => {
    ...
    assert.equal(b.charge('a', 1, 9, 5).source, 'actual');
    assert.equal(b.charge('b', undefined, 9, 5).source, 'metered');
    assert.equal(b.charge('c', undefined, 9).source, 'estimate');
  });

  it('섞이면 무엇이 섞였는지 누적 표시에 드러난다', () => {
    const b = new Budget(100);
    b.charge('a', 1, 9);
    b.charge('b', undefined, 9, 5);
